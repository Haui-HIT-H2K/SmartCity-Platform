<template>
  <div class="min-h-screen p-6 space-y-6">
    ...
    <div v-else-if="error" class="glass-card p-12 text-center">
      ...
      <h3 class="text-lg font-semibold text-gray-200 mb-2">Error Loading Record</h3>
      <p class="text-gray-500">{{ error.message || 'Failed to load record data' }}</p>
    ...
    <div v-else-if="record" class="space-y-6">
      ...
      <div class="glass-card p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div class="space-y-1">
            ...
            <p class="text-gray-200 font-mono text-sm break-all">{{ record.id }}</p>
          </div>
          <div class="space-y-1">
          ...
          <div class="space-y-1">
            ...
            <p class="text-gray-200 font-mono text-sm">{{ formatTimestamp(record.timestamp) }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="glass-card p-6">
        ...
        <div class="bg-dark-lighter rounded-lg p-4 border border-dark-border overflow-x-auto custom-scrollbar">
          <pre class="text-sm text-gray-300 font-mono">{{ formatJSON(record.payload) }}</pre>
        </div>
      </div>
      ...
      <div v-if="record.location" class="glass-card p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div class="space-y-1">
            ...
            <p class="text-gray-200 font-mono">{{ record.location.lat.toFixed(6) }}</p>
          </div>
          <div class="space-y-1">
            <label class="text-sm text-gray-500">Longitude</label>
            <p class="text-gray-200 font-mono">{{ record.location.lng.toFixed(6) }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="glass-card p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div class="space-y-1">
            ...
            <p class="text-gray-200 font-semibold text-lg">{{ record.value?.toFixed(2) || 'N/A' }}</p>
          </div>
          <div class="space-y-1">
            <label class="text-sm text-gray-500">Created At</label>
            <p class="text-gray-200 text-sm">{{ formatTimestamp(record.timestamp) }}</p>
          </div>
          <div class="space-y-1">
...
// Fetch record data
const { data: record, pending, error, refresh } = await useFetch(`/api/record/${id}`)
...
const getTypeBadgeClass = (type: DataType) => {
  ...
  return classes[type] || 'bg-gray-500/20 text-gray-400 border border-gray-500/40'
}
